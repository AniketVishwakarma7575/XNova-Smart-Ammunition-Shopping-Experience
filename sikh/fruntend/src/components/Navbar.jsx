...
<motion.div
      ...
      {/* --- Mobile Top Navbar --- */}
      <div className="md:hidden fixed top-0 left-0 right-0 bg-white shadow-md z-50 flex items-center justify-between px-4 py-2">
        {/* Left: Logo + App Name */}
        <Link
          to="/"
          className="flex items-center text-lg font-bold text-gray-800 tracking-tight"
        >
          <img
            src="https://th.bing.com/th/id/OIP.6mXNtWzQ5OSWbzlsurOQpgHaHa?w=198&h=198&c=7&r=0&o=7&dpr=1.3&pid=1.7&rm=3"
            alt="XNova"
          ...
          />
          <span className="text-base font-semibold">XNova</span>
        </Link>
        ...
        {user ? (
          ...
          >
            <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-700 shadow">
              <FaUser />
            </div>
            <motion.button
              onClick={logout}
             ...
             px-3 py-1 rounded-full shadow-md hover:from-pink-500 hover:to-indigo-500 
      ...
      >
        {navItems.map((item) => (
          ...
          >
            <motion.div whileTap={{ scale: 0.9 }} className="text-lg">
              {item.icon}
            </motion.div>
            <span>{item.name}</span>
          </Link>
